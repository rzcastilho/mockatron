defmodule MockatronWeb.RequestConditionController do
  ...
  def create(conn, %{
        "agent_id" => agent_id,
        "filter_id" => filter_id,
        "request_condition" => request_condition_params
      }) do
    current_user = Guardian.Plug.current_resource(conn)
    ...
    case Core.get_agent_from_user(agent_id, current_user) do
      ...
      agent ->
        case Core.get_filter_from_agent(filter_id, agent) do
          ...
          filter ->
            with {:ok, %RequestCondition{} = request_condition} <-
                   Core.create_request_condition(request_condition_params, filter) do
              conn
              |> put_status(:created)
              |> put_resp_header(
                "location",
        ...
        case Core.get_filter_from_agent(filter_id, agent) do
          ...
          filter ->
            case Core.get_request_condition_from_filter(id, filter) do
              ...
              request_condition ->
                with {:ok, %RequestCondition{} = request_condition} <-
                       Core.update_request_condition(request_condition, request_condition_params) do
                  render(conn, "show.json", request_condition: request_condition)
                end
            end
        end
        ...
        case Core.get_filter_from_agent(filter_id, agent) do
          ...
          filter ->
            case Core.get_request_condition_from_filter(id, filter) do
              ...
              request_condition ->
                with {:ok, %RequestCondition{}} <-
                       Core.delete_request_condition(request_condition) do
