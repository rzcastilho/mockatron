defmodule MockatronWeb.ResponseConditionController do
  ...
  def create(conn, %{"agent_id" => agent_id, "filter_id" => filter_id, "response_condition" => response_condition_params}) do
    current_user = Guardian.Plug.current_resource(conn)
    case Core.get_agent_from_user(agent_id, current_user) do
      nil ->
        conn
  ...
  def update(conn, %{"agent_id" => agent_id, "filter_id" => filter_id, "id" => id, "response_condition" => response_condition_params}) do
    current_user = Guardian.Plug.current_resource(conn)
    case Core.get_agent_from_user(agent_id, current_user) do
      nil ->
        conn
          ...
          filter ->
            case Core.get_response_condition_from_filter(id, filter) do
              nil ->
                ...
                |> json(%{error: "Not Found"})
              response_condition ->
                with {:ok, %ResponseCondition{} = response_condition} <- Core.update_response_condition(response_condition, response_condition_params) do
                  render(conn, "show.json", response_condition: response_condition)
                end
            end
        end
          ...
          filter ->
            case Core.get_response_condition_from_filter(id, filter) do
              nil ->
                ...
                |> json(%{error: "Not Found"})
              response_condition ->
                with {:ok, %ResponseCondition{}} <- Core.delete_response_condition(response_condition) do
