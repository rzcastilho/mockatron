defmodule MockatronWeb.FindCache do
  ...
  def call(
        %Plug.Conn{assigns: %{mockatron: %{agent: %{hash: hash} = agent} = mockatron}} = conn,
        :agent = cache
      ) do
  ...
  def call(
        %Plug.Conn{assigns: %{mockatron: %{responder: %{hash: hash} = responder} = mockatron}} =
          conn,
        :responder = cache
      ) do
    ...
    case Cachex.get(cache, hash) do
      ...
      {:ok, from_cache} ->
        ...
        |> assign(
          :mockatron,
          Map.merge(mockatron, %{
            cache => Map.merge(%{item | cache: true, found: true}, from_cache)
          })
        )
    end
  end
