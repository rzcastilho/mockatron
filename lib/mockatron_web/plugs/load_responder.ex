defmodule MockatronWeb.LoadResponder do
  ...
  def call(%Plug.Conn{assigns: %{mockatron: %{responder: %{cache: false, hash: hash, filter: filter} = responder, agent: %{agent: agent}} = mockatron}} = conn, _) do
    agent = case filter do
      nil -> agent
      [] -> agent
      ...
      %Mockatron.Core.Agent{responder: "RANDOM"} = agent ->
        case Random.start_link(agent) do
          {:ok, pid} ->
            ...
            conn
            |> assign(:mockatron, Map.merge(mockatron, %{responder: Map.merge(%{responder|found: true}, %{pid: pid, module: Mockatron.Responder.Random})}))
          _ ->
            conn
        end
      %Mockatron.Core.Agent{responder: "SEQUENTIAL"} = agent ->
        case Sequential.start_link(agent) do
          {:ok, pid} ->
            ...
            conn
            |> assign(:mockatron, Map.merge(mockatron, %{responder: Map.merge(%{responder|found: true}, %{pid: pid, module: Mockatron.Responder.Sequential})}))
          _ ->
            conn
        end
      nil ->
