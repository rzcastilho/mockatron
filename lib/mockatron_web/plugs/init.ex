defmodule MockatronWeb.Init do
  ...
  def call(
        %Plug.Conn{
          ...
          port: port,
          request_path: path,
          private: %{:guardian_default_claims => %{"sub" => user_id}}
        } = conn,
        _
      ) do
    protocol =
      case get_req_header(conn, @mockatron_target_protocol) do
        [] -> Atom.to_string(scheme)
        [target_protocol] -> target_protocol
      end

    host =
      case get_req_header(conn, @mockatron_target_hostname) do
        [] -> host
        [target_hostname] -> target_hostname
      end

    port =
      case get_req_header(conn, @mockatron_target_port) do
        [] -> port
    ...
    signature =
      case get_req_header(conn, "soapaction") do
        [] ->
          signature
    ...
    conn
    |> assign(:mockatron, %AgentResponder{
      signature: signature,
