defmodule Mockatron.Core.Agent do
    ...
    case changeset do
      %Ecto.Changeset{valid?: true, changes: %{path: path}} ->
        case Regex.match?(@regex_path_params_exists, path) do
          true ->
            path_regex =
              Regex.scan(@regex_path_params, path)
  ...
  def transform_path_param([snippet, name], path),
    do: String.replace(path, snippet, "(?<#{name}>[^/]+)")

  def transform_path_param([snippet, _, name, regex], path),
    do: String.replace(path, snippet, "(?<#{name}>#{regex})")
end
