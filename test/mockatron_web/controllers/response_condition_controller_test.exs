defmodule MockatronWeb.ResponseConditionControllerTest do
  ...
  describe "index" do
    test "lists all response_conditions", %{conn: conn, filter: filter} do
      conn =
        get(
          conn,
          Routes.agent_filter_response_condition_path(conn, :index, filter.agent_id, filter)
        )
    ...
    test "agent not found", %{conn: conn, filter: filter} do
      conn = get(conn, Routes.agent_filter_response_condition_path(conn, :index, 1000, filter))
  ...
  describe "show" do
    ...
    test "chosen response condition", %{
      conn: conn,
      filter: filter,
      response_condition: %ResponseCondition{id: id}
    } do
      conn =
        get(
          conn,
          Routes.agent_filter_response_condition_path(conn, :show, filter.agent_id, filter, id)
        )
    ...
    test "response condition not found", %{conn: conn, filter: filter} do
      conn =
        get(
          conn,
          Routes.agent_filter_response_condition_path(conn, :show, filter.agent_id, filter, 1000)
        )
    ...
    test "agent not found", %{
      ...
      conn = get(conn, Routes.agent_filter_response_condition_path(conn, :show, 1000, filter, id))
      assert response(conn, 404)
    end
  end
  ...
  describe "create response_condition" do
    test "renders response_condition when data is valid", %{conn: conn, filter: filter} do
      conn1 =
        post conn,
             Routes.agent_filter_response_condition_path(conn, :create, filter.agent_id, filter),
             response_condition: @create_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, filter: filter} do
      conn =
        post conn,
             Routes.agent_filter_response_condition_path(conn, :create, filter.agent_id, filter),
             response_condition: @invalid_attrs
    ...
    test "agent not found", %{conn: conn, filter: filter} do
      conn =
        post conn, Routes.agent_filter_response_condition_path(conn, :create, 1000, filter),
          response_condition: @create_attrs
  ...
  describe "update response_condition" do
    ...
    test "renders response_condition when data is valid", %{
      conn: conn,
      filter: filter,
      response_condition: %ResponseCondition{id: id} = response_condition
    } do
      conn1 =
        put conn,
            Routes.agent_filter_response_condition_path(
            ...
            response_condition: @update_attrs
  ...
  describe "delete response_condition" do
    ...
    test "deletes chosen response_condition", %{
      conn: conn,
      filter: filter,
      response_condition: response_condition
    } do
