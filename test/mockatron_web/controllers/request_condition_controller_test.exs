defmodule MockatronWeb.RequestConditionControllerTest do
  ...
  def fixture(:request_condition) do
    filter =
      Repo.one(
        from a in Filter,
          select: a,
          where: a.label == "success" and a.priority == 0 and a.enable == true
      )
  ...
  describe "index" do
    test "lists all request_conditions", %{conn: conn, filter: filter} do
      conn =
        get(
          conn,
          Routes.agent_filter_request_condition_path(conn, :index, filter.agent_id, filter)
        )
    ...
    test "agent not found", %{conn: conn, filter: filter} do
      conn = get(conn, Routes.agent_filter_request_condition_path(conn, :index, 1000, filter))
  ...
  describe "show" do
    ...
    test "chosen request condition", %{
      conn: conn,
      filter: filter,
      request_condition: %RequestCondition{id: id}
    } do
      conn =
        get(
          conn,
          Routes.agent_filter_request_condition_path(conn, :show, filter.agent_id, filter, id)
        )
    ...
    test "request condition not found", %{conn: conn, filter: filter} do
      conn =
        get(
          conn,
          Routes.agent_filter_request_condition_path(conn, :show, filter.agent_id, filter, 1000)
        )
    ...
    test "filter not found", %{
      ...
      conn =
        get(
          conn,
          Routes.agent_filter_request_condition_path(
            conn,
            :show,
            filter.agent_id,
            %Filter{id: 1000},
    ...
    test "agent not found", %{
      ...
      conn = get(conn, Routes.agent_filter_request_condition_path(conn, :show, 1000, filter, id))
      assert response(conn, 404)
    end
  end
  ...
  describe "create request_condition" do
    test "renders request_condition when data is valid", %{conn: conn, filter: filter} do
      conn1 =
        post conn,
             Routes.agent_filter_request_condition_path(conn, :create, filter.agent_id, filter),
             request_condition: @create_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, filter: filter} do
      conn =
        post conn,
             Routes.agent_filter_request_condition_path(conn, :create, filter.agent_id, filter),
             request_condition: @invalid_attrs
    ...
    test "agent not found", %{conn: conn, filter: filter} do
      conn =
        post conn, Routes.agent_filter_request_condition_path(conn, :create, 1000, filter),
          request_condition: @create_attrs
  ...
  describe "update request_condition" do
    ...
    test "renders request_condition when data is valid", %{
      conn: conn,
      filter: filter,
      request_condition: %RequestCondition{id: id} = request_condition
    } do
      conn1 =
        put conn,
            Routes.agent_filter_request_condition_path(
            ...
            request_condition: @update_attrs
  ...
  describe "delete request_condition" do
    ...
    test "deletes chosen request_condition", %{
      conn: conn,
      filter: filter,
      request_condition: request_condition
    } do
